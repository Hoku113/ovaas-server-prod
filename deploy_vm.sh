...
function deploy_vm () {
    ...
    if [ $DEPLOY_TYPE = "vm" ]; then
        ...
        COMMAND=$(echo '{"fileUris":["https://raw.githubusercontent.com/OVaaS/ovaas-server-prod/main/CUSTOM_SCRIPT_setup_ovms.sh"],"commandToExecute":"./CUSTOM_SCRIPT_setup_ovms.sh \"'${AZURE_STORAGE_CONNECTION_STRING}'\" '${PARAM_FOR_CUSTOM_SCRIPT}'"}')
        echo $COMMAND
